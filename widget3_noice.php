...
<?php
	...
	$key = "NmNhNGFiMjVkYWIwOTNjNzdjMDg";
	$secret = "OThlZDEyYzA5MTE4MTY4OWM5ZDQ";
	
	$oauth = $api->oauthLogin($key, $secret);
	
	if($oauth){
		
		$stmt = $pdo->query("SELECT * FROM `widget_config` WHERE user_id = '".$api->readCurrentUser()->getID()."' AND `session_key` = 'widget3_noise_deviceid' LIMIT 1;");
		...
		if(isset($_GET['deviceid']) && $obj == null){
			$pdo->exec("INSERT INTO `widget_config`(id, session_key, session_value, user_id) VALUES (NULL, 'widget3_noise_deviceid', '".$_GET['deviceid']."','".$api->readCurrentUser()->getID()."');");
		...
		if(($deviceid == 0) || (isset($_GET['settings']) && $_GET['settings'] == 'true')){
			...
			if(isset($sensor) && $sensor != null){
				$dataarray = $sensor->getData(0,100,$fromtime,$endtime,0,0,0,'ASC',FALSE,$interval);
			}
		}
...
<script type="text/javascript">
      ...
      function drawVisualization() {
		...
		<?php 
			if(isset($dataarray) && $dataarray != ""){
				foreach ($dataarray as $data) {
					echo "data.addRow([new Date(". $data->getDate()*1000 ."), ".$data->getValue()."]);";
				}
			}
		?>
        ...
        options = {
          'width':  '100%', 
          <?php if(isset($_GET['size'])){echo "'height': '130px' ";}else{echo "'height': '300px' ";}?>,
