<?php
	...
	$key = "Y2NjZTg2N2ZiNmVjNGFlYTVhZWI";
	$secret = "YzQ0ODk1NzNjYTQ2NWQ0ZDdiN2E";
	
	$oauth = $api->oauthLogin($key, $secret);
	//$api->setSessionId($_GET['sessionid']);
	
	if($oauth){
		
		$stmt = $pdo->query("SELECT * FROM `widget_config` WHERE user_id = '".$api->readCurrentUser()->getID()."' AND `session_key` = 'widget2_top_10_deviceid' LIMIT 1;");
		...
		if(isset($_GET['deviceid']) && $obj == null){
			$pdo->exec("INSERT INTO `widget_config`(id, session_key, session_value, user_id) VALUES (NULL, 'widget2_top_10_deviceid', '".$_GET['deviceid']."','".$api->readCurrentUser()->getID()."');");
